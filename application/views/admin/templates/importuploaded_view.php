...
<div class='messagebox ui-corner-all'>
        ...
        <ul>
            <?php
                foreach ($aImportedFilesInfo as $entry)
                {
                    if ($entry['is_folder']){
                    ?> 
                    <li><?php echo gT("Folder") . ": " . $entry["filename"] ?></li>
                    <?php
                    }
                    else
                    { ?>
    ...
    ?>
    <input type='submit' value='<?php eT("Open imported template") ?>' onclick="window.open('<?php echo $this->createUrl('admin/templates/view/editfile/startpage.pstpl/screenname/welcome/templatename/' . $newdir) ?>', '_top')" />
</div>

<?php if (isset($step1))
    ...
    <br /><div class='messagebox ui-corner-all'>
        ...
        <p><?php eT("There are two ways to stop a survey. Please read carefully on the two options below and choose the right one for you."); ?></p>
        <table><tr><th width='50%'><?php eT("Expiration"); ?></th><th><?php eT("Deactivation"); ?></th></tr>
            <tr><td><ul>
                ...
                <td>
                    <?php echo CHtml::form(array("admin/survey/sa/expire/surveyid/{$surveyid}/"), 'post'); ?>
                        <input type='submit' value='<?php eT("Expire survey"); ?>'/>
                    </form>
                </td>
                <td>
                    <?php echo CHtml::form(array("admin/survey/sa/deactivate/surveyid/{$surveyid}/"), 'post'); ?>
                        <input type='submit' value='<?php eT("Deactivate survey"); ?>' onclick="<?php echo convertGETtoPOST($this->createUrl("admin/survey/sa/deactivate/".$surveyid)."?action=deactivate&amp;ok=Y&amp;sid=$surveyid"); ?>" />

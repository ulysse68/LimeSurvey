...
if (isset($tmp_survlangs)) { ?>
    <div class="langpopup" id="previewquestionpopup"><?php eT("Please select a language:"); ?><ul>
...
<div class='menubar-main'>
    <div class='menubar-left'>
        ...
        <?php if(hasSurveyPermission($surveyid,'surveycontent','read'))
            {
                ...
                <a accesskey='q' id='previewquestion'>
                    <img src='<?php echo $sImageURL; ?>preview.png' title='' alt='<?php eT("Preview This Question"); ?>' /></a>
                <img src='<?php echo $sImageURL; ?>separator.gif' class='separator' alt=''  />
        ...
        <?php if ($activated != "Y")
            {?>
            <a href='#'
                onclick="if (confirm('<?php eT("Deleting this question will also delete any answer options and subquestions it includes. Are you sure you want to continue?","js"); ?>')) { <?php echo convertGETtoPOST($this->createUrl("admin/question/sa/delete/surveyid/$surveyid/gid/$gid/qid/$qid")); ?>}">
                <img style='<?php echo (hasSurveyPermission($surveyid,'surveycontent','delete')?'':'visibility: hidden;');?>' src='<?php echo $sImageURL; ?>delete.png' alt='<?php eT("Delete current question"); ?>'/></a>
            <?php }
            else
            { ?>
        ...
        <?php if(hasSurveyPermission($surveyid,'surveycontent','create'))
            ...
            if(hasSurveyPermission($surveyid,'surveycontent','read'))
            {
                ...
                <a href='<?php echo $this->createUrl('admin/question/sa/subquestions/surveyid/'.$surveyid.'/gid/'.$gid.'/qid/'.$qid); ?>'>
                    <img src='<?php echo $sImageURL; ?><?php if ($qproperties['subquestions']==1){?>subquestions.png<?php } else {?>subquestions2d.png<?php } ?>' alt='<?php eT("Edit subquestions for this question"); ?>' /></a>
                <?php }
            }
            else
            { ?>
...
<table  id='questiondetails' <?php echo $qshowstyle; ?>><tr><td><strong>
    ...
    <tr><td><strong>
            <?php eT("Type:"); ?></strong></td><td><?php echo $qproperties['description']; ?>
        </td></tr>
    <?php if ($qct == 0 && $qproperties['answerscales'] >0)
        { ?>
        <tr ><td></td><td>
                <span class='statusentryhighlight'>
                    <?php eT("Warning"); ?>: <a href='<?php echo $this->createUrl("admin/question/sa/answeroptions/surveyid/$surveyid/gid/$gid/qid/$qid"); ?>'><?php eT("You need to add answer options to this question"); ?>
        ...
        <tr ><td></td><td>
                <span class='statusentryhighlight'>
                    <?php eT("Warning"); ?>: <a href='<?php echo $this->createUrl("admin/question/sa/subquestions/surveyid/$surveyid/gid/$gid/qid/$qid"); ?>'><?php eT("You need to add subquestions to this question"); ?>
                        <img src='<?php echo $sImageURL; ?><?php if ($qproperties['subquestions']==1){?>subquestions_20<?php } else {?>subquestions2d_20<?php } ?>.png' title='<?php eT("Edit subquestions for this question"); ?>' /></a></span></td></tr>
        <?php }

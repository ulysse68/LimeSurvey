<div class='messagebox ui-corner-all'>
    <div class='successheader'><?php eT("Uploaded CSV file successfully"); ?></div><br />
    <?php if (empty($tokenlistarray)) { ?>
        <div class='warningheader'><?php eT("Failed to open the uploaded file!"); ?></div><br />
        <?php } ?>
    <?php if (!in_array('firstname', $firstline) || !in_array('lastname', $firstline) || !in_array('email', $firstline)) { ?>
        <div class='warningheader'><?php eT("Error: Your uploaded file is missing one or more of the mandatory columns: 'firstname', 'lastname' or 'email'"); ?></div><br />
        <?php } ?>
    <?php if ($xz != 0) { ?>
        <div class='successheader'><?php eT("Successfully created token entries"); ?></div><br />
        <?php } else { ?>

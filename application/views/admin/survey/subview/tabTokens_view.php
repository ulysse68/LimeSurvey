<div id='tokens'><ul>
        ...
        <li>
            <label for='alloweditaftercompletion'><?php eT("Allow editing responses after completion?"); ?></label>
            <select id='alloweditaftercompletion' name='alloweditaftercompletion' onchange="javascript: if (document.getElementById('private').value == 'Y') { alert('<?php eT("This option can't be set if Anonymous answers are used","js"); ?>'); this.value='N';}">
                <option value='Y'
        ...
        <li><label for='directregister'><?php eT("Allow entering survey without mail validation?"); ?></label>
            <select id='directregister' name='directregister'>
                <option value='N'
                    <?php if ($esrow['directregister'] != "Y" && $esrow['directregister'] != "A") { ?>
                        ...
                        <?php } ?>
                    ><?php eT("No"); ?></option>
                <option value='Y' title='<?php eT("Yes: the url are shown in message."); ?>' 
                    <?php if ($esrow['directregister'] == "Y") { ?>
                        selected='selected'
                        <?php } ?>
                    ><?php eT("Yes: shown"); ?></option>
                <option value='A' title='<?php eT("Yes: enter directly into the survey after registering."); ?>' 
                    <?php if ($esrow['directregister'] == "A") { ?>
                        selected='selected'
                        <?php } ?>
                    ><?php eT("Yes: direct"); ?></option>
            </select>
            <!--<span class="annotation"><?php eT("User can enter without mail validation. Please ensure legality of user registration without validation process in your country."); ?></span>-->
        </li>
